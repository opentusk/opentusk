...
function writeRichText(rte, html, width, height, buttons, readOnly) {
	...
	//		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'redo.gif" width="25" height="24" alt="Redo" title="Redo" onClick="rteCommand(\'' + rte + '\', \'redo\')"></td>');
		...
		document.writeln('<iframe width="154" height="104" id="cp' + rte + '" src="' + includesPath + 'palette.htm" marginwidth="0" marginheight="0" scrolling="no" style="visibility:hidden; position: absolute; top:0;"></iframe>');
...
function setHiddenVal(rte) {
	...
	if (document.all) {
		if (generateXHTML) {
			...
			oHdnField.value = oHdnField.value.replace(/^\s+/g,'');
			oHdnField.value = oHdnField.value.replace(/\s+$/g,'');
		}
	} else {
		if (generateXHTML) {
