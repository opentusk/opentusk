...
<xsl:for-each select="SLIDE">
	<xsl:if test="(position() mod 2) = 1">
		...
		<fo:table-row height="68mm" keep-with-previous="always">
		...
		<fo:block>
			<fo:external-graphic height= "66mm" src="{@SRC}"/>
		</fo:block>
		</fo:table-cell>
		<fo:table-cell>
		</fo:table-cell>
		...
		<fo:block>
			<xsl:if test="not((position() + 1) &gt; last())">
			<fo:external-graphic height= "66mm"  src="{following-sibling::SLIDE[1]/attribute::SRC}"/>
			</xsl:if>
		</fo:block>
		</fo:table-cell>
		</fo:table-row>
