    ...
    unless ($self->{-user_codes}) {
        ...
        if ($self->parent_eval()->is_teaching_eval()) {
            ...
            my $sql = qq(
                ...
                WHERE eval_id IN ($eval_ids) AND evaluatee_id $op1 ? AND teaching_site_id $op2 ?
            );
            eval {
                my $sth = $dbh->prepare($sql);
                $sth->execute($evaluatee_id, $teaching_site_id);
