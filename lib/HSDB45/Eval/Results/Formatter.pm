...
sub user_code_labeler {
    ...
    $self->{-user_code_letters}{$user_code} = ($self->{-user_number} == 0)
        ? $self->{-user_letter} : $self->{-user_number} . $self->{-user_letter};
    ...
    if ($interpreted_response) {
        my $response_elt = XML::Twig::Elt->new("Response",
                                               {"user_token" => $response->user_code(),
                                                "pretty_user_label" => $self->user_code_labeler($response->user_code())},
