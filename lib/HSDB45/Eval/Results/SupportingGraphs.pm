...
package HSDB45::Eval::Results::SupportingGraphs;
...
my $tablename = "eval_results_supporting_graphs";
my $primary_key_field = "eval_results_support_graph_id";
my @fields = qw(eval_results_support_graph_id eval_id eval_question_id graph_type categorization_question_id categorization_value mime_type width height graphic graphic_text modified);
my %blob_fields = (graphic => 1, graphic_text => 1);
my %numeric_fields = (width => 1, height => 1);

my %cache = ();

# Description: creates a new HSDB45::Eval::Results::SupportingGraphs object
...
# Description: Gets the eval_results_support_graph ID
# Input: question_id
# Output: The eval_results_support_graph_id
sub get_id {
    my $self = shift;
    my $eval_id = shift;
    ...
    my $sth = $dbh->prepare(qq[SELECT eval_results_support_graph_id 
			       FROM $school_db\.eval_results_supporting_graphs 
			       WHERE eval_id=? AND eval_question_id=? AND graph_type=?]);
