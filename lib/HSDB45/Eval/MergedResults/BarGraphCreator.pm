package HSDB45::Eval::MergedResults::BarGraphCreator;
...
my @mod_deps  = ('HSDB45::Eval::MergedResults',
		 'HSDB45::Eval::MergedResults::BarGraph',
...
# Output: Blessed, initialized HSDB45::Eval::MergedResults::BarGraphCreator object
sub new {
    my $class = shift;
...
sub init {
    my $self = shift;
    $self->{-school} = shift;
    $self->{-merged_eval_results_id} = shift;
    $self->{-xml_doc} =  shift;
    my $merged_results =
      HSDB45::Eval::MergedResults->new(_school => $self->{-school},
                                       _id => $self->{-merged_eval_results_id});
    my $eval = $merged_results->parent_eval();
...
# Input:
# Output: The ID
sub get_merged_eval_results_id {
    my $self = shift;
    return $self->{-merged_eval_results_id};
}
...
sub save_svg_graphs {
	...
	my $blank_bar = HSDB45::Eval::MergedResults::BarGraph->new(_school => $self->get_school());
	my $blank_histo = HSDB45::Eval::MergedResults::SupportingGraphs->new(_school => $self->get_school());
	my $blank_median = HSDB45::Eval::MergedResults::SupportingGraphs->new(_school => $self->get_school());
	...
	foreach my $questionResult (@results) {
			...
			if($categorizationResponses->size() == 0) {
				...
				foreach my $response ($categorizationResponses->get_nodelist()) {
					...
					if($group) {
						...
						${$tempHash}{minCategories} = $minXValue;
						${$tempHash}{maxCategories} = $maxXValue;
		...
		if($nodeType eq 'PlusMinusRating') {
			...
			} elsif($mean) {$mean = $questionResult->findvalue('ResponseGroup/ResponseStatistics/mean');
				# Were we a simple graph?
		...
		my $id = $blank_bar->get_id($self->get_merged_eval_results_id(), $questionID);
		if($id)	{
			my $sthu;
			if($mimeType) {
				$sthu = $dbh->prepare(qq[UPDATE $school_db\.eval_mergedresults_graphics SET graphic=?, mime_type='$mimeType', graphic_text=?  WHERE eval_mergedresults_graphics_id=?]);
			} else {
				$sthu = $dbh->prepare(qq[UPDATE $school_db\.eval_mergedresults_graphics SET graphic=?, graphic_text=? WHERE eval_mergedresults_graphics_id=?]);
			}
			my $r = $sthu->execute($output, $textOutput,$id);
		} else {
			my $sthi = $dbh->prepare(qq[INSERT INTO $school_db\.eval_mergedresults_graphics (merged_eval_results_id, eval_question_id, graphic, mime_type, graphic_text) VALUES (?, ?, ?, ?, ?)]);
			$sthi->execute($self->get_merged_eval_results_id(), $questionID, $output, $mimeType, $textOutput);
		}
		...
		my $medianId = $blank_histo->get_id($self->get_merged_eval_results_id(), "$questionID", "median");
		if($medianId)    {
			my $sthu;
			# Medians are new, so we can just always set the new mime type
			$sthu = $dbh->prepare(qq[UPDATE $school_db\.eval_merged_results_supporting_graphs SET graphic=?, mime_type=?, graphic_text=?  WHERE eval_merged_results_support_graph_id=?]);
			my $r = $sthu->execute($medianGraphOutput, $mimeType, $textMedianGraphOutput, $medianId);
		} else {
			my $sthi = $dbh->prepare(qq[INSERT INTO $school_db\.eval_merged_results_supporting_graphs (merged_eval_results_id, eval_question_id, graph_type, graphic, mime_type, graphic_text) VALUES (?, ?, "median", ?, ?, ?)]);
			$sthi->execute($self->get_merged_eval_results_id(), "$questionID", $medianGraphOutput, $mimeType, $textMedianGraphOutput);
		}
		...
		#	my $modeId = $blank_histo->get_id($self->get_merged_eval_results_id(), "$questionID", "mode");
		#	if($modeId)    {
		#		my $sthu;
		#		# Medians are new, so we can just always set the new mime type
		#		$sthu = $dbh->prepare(qq[UPDATE $school_db\.eval_merged_results_supporting_graphs SET graphic=?, mime_type=?  WHERE eval_merged_results_support_graph_id=?]);
		#		my $r = $sthu->execute($modeGraphOutput, $mimeType, "$modeId");
		#	} else {
		#		my $sthi = $dbh->prepare(qq[INSERT INTO $school_db\.eval_merged_results_supporting_graphs (merged_eval_results_id, eval_question_id, graph_type, graphic, mime_type) VALUES (?, ?, "mode", ?, ?)]);
		#		$sthi->execute($self->get_merged_eval_results_id(), $questionID, $modeGraphOutput, $mimeType);
		#	}
	}
	...
	foreach my $questionID (keys %histograms) {
		...
		my $histoID = $blank_histo->get_id($self->get_merged_eval_results_id(), "$questionID", "histogram");
		if($histoID)	{
			my $sthu;
			# Histograms are new, so we can just always set the new mime type
			$sthu = $dbh->prepare(qq[UPDATE $school_db\.eval_merged_results_supporting_graphs SET graphic=?, graphic_text=?, mime_type=?  WHERE eval_merged_results_support_graph_id=?]);
			my $r = $sthu->execute($histoOutput, $histoOutputText, $mimeType, $histoID);
		} else {
			my $sthi = $dbh->prepare(qq[INSERT INTO $school_db\.eval_merged_results_supporting_graphs (merged_eval_results_id, eval_question_id, graph_type, graphic, mime_type, graphic_text) VALUES (?, ?, "histogram", ?, ?, ?)]);
			$sthi->execute($self->get_merged_eval_results_id(), "$questionID", $histoOutput, $mimeType, $histoOutputText);
		}
