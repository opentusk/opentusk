...
sub find_users {
    ...
    return TUSK::Core::HSDB4Tables::User->lookup($conditions, $sort_orders, undef, undef, [
		   TUSK::Core::JoinObject->new('TUSK::Course::User', { joinkey => 'user_id', jointype => 'inner', joincond =>"course_id = " . $self->primary_key() . " AND school_id = " . $school->getPrimaryKeyID() . (($tp_id > 0) ? " AND time_period_id IN ($tp_id)" : '') }),
