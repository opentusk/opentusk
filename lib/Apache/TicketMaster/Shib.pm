...
sub authenticate {
	...
	my $request_uri = '/home';
		if ($apr->param('request_uri')) {
			$request_uri = $apr->param('request_uri');
			} elsif ($cookieJar->cookies('request_uri')){
				$request_uri = $cookieJar->cookies('request_uri')->value();
			} elsif($r->prev()) {
				$request_uri = $r->prev()->uri();
			}		
