...
my ($DB_USER, $DB_PASSWORD, $script_user, $logfile, $DB_HOST);
my ($dbh, $sth, $sql, $retval);
my @tusk_dbs;
my $skip_tables = {
					'tusk.link_search_query_content' => 1,
					'hsdb4.content_history'          => 1,
				  };
...
sub setupDBConnection {
	...
	#########
	print "\nPlease enter username of a MYSQL account with ALTER permissions. Hint: the default is often 'root'.\n\nUser name: ";
	$user_name = <>;
	chomp $user_name;
...
sub findTuskDBs {
	@tusk_dbs = ('tusk', 'hsdb4');
	my $sql = "SHOW DATABASES LIKE '%hsdb45%'"; 
	$sth = $dbh->prepare($sql);
	eval { $sth->execute() };
	die "Failure: $@ - sql: $sql" if ($@);
...
sub alterTables {
	...
	foreach my $table (keys(%$queries)) {
		...
		print $logfile sprintf("Total conversion time: %u seconds\n", (time - $starttime));
	}	
...
sub alterIndexedTables {
	...
	TABLE_LOOP: foreach my $table (keys(%$indexed_tables)) {
		...
		my $table_conversion_query->{"$db_name.$table_name"} = $indexed_tables->{$table};
