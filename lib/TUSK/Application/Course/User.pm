...
sub add {
    ...
    _updateUserSites({ %$params, course_user_id => $course_user->getPrimaryKeyID() }, []);
...
sub _updateUserSites {
	...
	foreach my $course_user_site (@$course_user_sites) {
	    ...
	    if ($delete_all || !exists $new_site_ids{$existing_site_id}) {
		$to_be_deleted{$existing_site_id} = $course_user_site;
	    } 
	    delete $new_site_ids{$existing_site_id};
	}

	foreach my $course_user_site (values %to_be_deleted) {
	    if (my $new_site_id = (keys %new_site_ids)[0]) {
		$course_user_site->setTeachingSiteID($new_site_id);
		$course_user_site->save({user => $params->{author}});
		delete $new_site_ids{$new_site_id};
	    } else {
