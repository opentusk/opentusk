...
package TUSK::AcademicLevel::TimePeriodYear;

=head1 NAME

B<TUSK::AcademicLevel::TimePeriodYear> - Class for manipulating entries in table time_period_year in tusk database
...
sub new {
    ...
    my $self = $class->SUPER::new ( 
				    _datainfo => {
					'database' => 'tusk',
					'tablename' => 'time_period_year',
					'usertoken' => '',
					'database_handle' => '',
					},
				    _field_names => {
					'time_period_year_id' => 'pk',
					'school_id' => '',
					'time_period_id' => '',
					'year_id' => '',
				    },
...
my $string = $obj->getYearID();

Get the value of the year_id field

=cut

sub getYearID{
    my ($self) = @_;
    return $self->getFieldValue('year_id');
}
...
Set the value of the year_id field

=cut

sub setYearID{
    my ($self, $value) = @_;
    $self->setFieldValue('year_id', $value);
}
