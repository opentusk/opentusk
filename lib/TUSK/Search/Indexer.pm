...
B<TUSK::Search::Indexer> - Class for manipulating entries in table search_query in tusk database
...
The method takes two parameters: a content object and an optional flag that indicates
whether to search the content for UMLS Concepts.  A true value will cause the 
content to be searched, with no value passed the content is not searched.
...
sub indexContent{
	...
	if ($contentObject->type() eq 'Collection'){
	    ...
	    $contentObject->field_value('title', "$title " x 5); # give more weight to titles of collections
	}
	my $contentIndex = TUSK::Search::Content->new()->lookupReturnOne("content_id = " . $contentObject->primary_key());
	...
	my $keyword_links = TUSK::Core::LinkContentKeyword->new()->lookup("parent_content_id = " . $contentObject->primary_key());
	...
	if (-e $TUSK::Constants::mmtxExecutable && $UMLSIndex && !scalar(@$umls_concept_mentions)){
	    ...
	    $umls_concept_mentions = TUSK::UMLS::UmlsConceptMention->lookup("content_id = '" . $contentObject->getPrimaryKeyID() . "'", ['map_weight desc'], undef, 5);
	}

	if ($use_suggested_concepts){
	    foreach my $umls_concept_mention (@$umls_concept_mentions){
		push (@$keywords, $umls_concept_mention->getKeywordObject()->getConceptID());
	    }
	}
	...
	if ($contentObject->type() eq 'External') {
	    if (my $metadata = TUSK::Content::External::MetaData->lookupReturnOne("content_id = " . $contentObject->primary_key())) {
...
sub unindexContent {
	...
	foreach my $content_id (@_) {
	    ...
	    my $content_search = TUSK::Search::Content->new()->lookupReturnOne("content_id = " . $content_id);
...
sub UMLSIndexContent{
	...
	my $delete = TUSK::UMLS::UmlsConceptMention->new()->delete("content_id = '" . $content->primary_key() . "'");

	foreach my $concept (@$concepts) {

		next if (! $concept->{concept} );

		my $keyword = TUSK::Core::Keyword->lookup( "concept_id = '" . $concept->{concept} . "'" );

		if (! @$keyword ){
		    
		    next;
			#confess "The concept id " . $concept->{concept} . " is not associated to a valid keyword";
		}
