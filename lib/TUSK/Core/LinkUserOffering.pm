...
package TUSK::Core::LinkUserOffering;

=head1 NAME

B<TUSK::Core::LinkUserOffering> - Class for manipulating entries in table link_user_offering in tusk database
...
sub new {
    ...
    my $self = $class->SUPER::new ( 
				    _datainfo => {
					'database' => 'tusk',
					'tablename' => 'link_user_offering',
					'usertoken' => '',
					'database_handle' => '',
					},
				    _field_names => {
					'link_user_offering_id' => 'pk',
					'offering_id' => '',
					'user_id' => '',
					'role_id' => '',
					'sort_order' => '',
				    ...
				    _default_join_objects => [
							    TUSK::Core::JoinObject->new("TUSK::Core::Role"),
							    TUSK::Core::JoinObject->new("TUSK::Core::User"),
							      ],
				    @_
				  );
...
=item B<getRoleObject>

    $obj = $obj->getRoleObject();

    Get the role join object

=cut

sub getRoleObject{
    my ($self) = @_;
    return $self->getJoinObject("TUSK::Core::Role");
}
...
=item B<getUserObject>
    ...
    $obj = $obj->getUserObject();

    Get the user join object

=cut

sub getUserObject{
    my ($self) = @_;
    return $self->getJoinObject("TUSK::Core::User");
}
