...
TUSK::Core::Plugins - The great new TUSK::Core::Plugins!
...
sub _init {
     ...
     $this->plugin_dirs($args->{plugin_dirs}) 	if(exists $args->{plugin_dirs});
	...
	if( $pkg_name ) {
		...
		if($pkg_name->can('new')) {
			...
			if( ref $o eq $pkg_name) {
               ...
               # $this->_add_plugin({ obj => $o, name => $pkg_name, path => $pkg_path }) if($valid);
...
## Find valid plugin search directories based on defaults and args.
sub _get_plugin_dirs {
	my $this = shift;
	if($this->tot_plugin_dirs) {
		my @dirs = $this->plugin_dirs;;
		foreach my $dir (@dirs) {
...
||||||| parent of 0798d09... Added Plugin module per [TUS-TUS-2481]
=======
# Copyright 2012 Tufts University 
#
# Licensed under the Educational Community License, Version 1.0 (the "License"); 
...
1; # End of TUSK::Core::Plugins
>>>>>>> 0798d09... Added Plugin module per [TUS-TUS-2481]
