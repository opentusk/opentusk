...
=item B<exportToGradeBook>
    ...
    ($msgs, $warnings) = $quiz->exportToGradeBook($grade_event, $commit);
...
$msgs is a list of strings for a user to see what the export did and
did not do. $warnings is a list of warnings generated by the export.
...
sub exportToGradeBook{
    ...
    # Check grade event is valid for this quiz
    if (!defined($grade_event_quiz_id) || ($grade_event_quiz_id eq
                                           $self->getPrimaryKeyID())) {
    ...
    my ($link, $links, $grade_event_id, $user_object, $user_id, $updatingLink,
        $score, $user_name, $old_score);
    my @msg = ();
    my @warnings = ();
    my $record_count = 0;
    $grade_event_id = $grade_event->getPrimaryKeyID();

    # check for unsubmitted quiz results
    my @unsubQuizzes = grep {! defined $_->getEndDate()} @$results;
    ...
    foreach my $result (@{$results}) {
        ...
        $links = TUSK::GradeBook::LinkUserGradeEvent->lookupByRelation(
            $user_id, $grade_event_id);
        if (scalar(@{$links})) {
            $link = pop @{$links};
            $updatingLink = 1;
        ...
        if ($commit){
            if ($updatingLink && ($old_score ne $score)) {
                push @msg, "Changing Grade for $user_name " .
                    "from $old_score to $score";
            }
            $link->save({'user'=>$self->getUser()});
        } else {
            if ($updatingLink && ($old_score ne $score)){
                push @msg, "The grade for $user_name would change " .
                    "from $old_score to $score";
            }
        }
    }
    if ($commit){
        push @msg, "Export complete -- $record_count records exported";
    } else {
        push @msg, "Test complete -- $record_count records would be exported.";
    }
    if (!scalar(@{$results})){
        push @msg, 'There are no eligible results to export for this quiz.';
    }
    return (\@msg, \@warnings);
}
...
sub getExportableResults {
    ...
    return [] unless (scalar(@students)); # no students, so no results
    my $dbh = $self->getDatabaseReadHandle();
    my $sqlStudentCond = 'user_id IN (' .
        join(", ", map { $dbh->quote($_->primary_key()) } @students) .
        ')';
    my $quiz_id = $self->getPrimaryKeyID();
    my $sqlLookup = " quiz_id = " . $dbh->quote($quiz_id) .
        " AND $sqlStudentCond ";
    my $results = TUSK::Quiz::Result->lookup($sqlLookup);
    my @exportable_results = @{$results};
